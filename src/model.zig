...
// ── Weight loading (MLX-backed; see src/model_weights.zig) ─────────────────
//
// `Weights` is a map of `mlx_array`, so it only exists where MLX does. The
// config half above is portable and is what a GGUF-only build actually uses:
// ModelConfig carries kvBytesPerToken and the prefill-billing helpers that the
// admission guard and the auto-context sizer read, none of which touch MLX.
const weights_impl = if (@import("build_cfg.zig").mlx_enabled)
    @import("model_weights.zig")
else
    @import("model_weights_stub.zig");

pub const Weights = weights_impl.Weights;
pub const resolveWeightPrefix = weights_impl.resolveWeightPrefix;
pub const loadWeights = weights_impl.loadWeights;
pub const loadWeightsSingleFile = weights_impl.loadWeightsSingleFile;
pub const loadWeightsWithVision = weights_impl.loadWeightsWithVision;
pub const reportF16Narrowing = weights_impl.reportF16Narrowing;
pub const loadSafetensorsFile = weights_impl.loadSafetensorsFile;
pub const narrowsLoadedF16 = weights_impl.narrowsLoadedF16;
